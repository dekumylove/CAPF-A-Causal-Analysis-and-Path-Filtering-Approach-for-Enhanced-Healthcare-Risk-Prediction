...
class GraphCare(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, num_rel=11, num_feat=2850, num_visit=6, reduced_dim=256, gamma=0.1, dropout=0.3, max_feat=10, bidirectional=False, device=torch.device('cuda')):
    ...
    def forward(self, feat_index, neighbor_index, rel_index):
        ...
        output = self.output_layer(self.final_mlp(z_joint))
